...
export const getColumns = ({
  ...
  // },
  {
    title: (
      <HeaderCell
        title={
          <span className="whitespace-nowrap">{t?.("table-text-teams")}</span>
        }
      />
    ),
    dataIndex: "teams",
  ...
  },
  {
    title: (
      <HeaderCell
        className="ps-3"
        title={
          <span className="whitespace-nowrap">{t?.("table-text-actions")}</span>
        }
      />
    ),
    dataIndex: "action",
    ...
    render: (_: string, row: any) => (
      <div className="flex items-center justify-end gap-3 pe-4">
        <Tooltip
          size="sm"
          content={t?.("table-text-edit-member")}
          placement="top"
          color="invert"
        >
        ...
        <DeletePopover
          title="table-text-delete-member"
          description={`${t?.("table-text-delete-confirm-shipment")} #${row.id} ${t?.("table-text-member")}?`}
