...
function InfoAccordion( {lang}:{lang?: string })  {
  const [info, setInfo] = useState<any[]>([]);
  const { t } = useTranslation(lang!, 'home');

  useEffect(() => {
    const fetchData = async () => {
      const data = await getInfoAccordion({lang});
  ...
  return (
    <div className={`bg-[#F8FAFC] md:h-auto relative font-montserrat`}>
      <div className={`${style.layer} dark:bg-mainBg bg-[#F8FAFC] h-auto`}>
        <div className="md:w-9/12 w-11/12 mx-auto 4xl:w-9/12 flex justify-center lg:justify-between ">
          <div className="">
