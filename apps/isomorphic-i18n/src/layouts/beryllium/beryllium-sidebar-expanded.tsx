...
function LinkMenuItem({ item, lang }: { item: ItemType; lang?: string }) {
  const { t } = useTranslation(lang!, "nav");
  const { colorPresetName } = useColorPresetName();
  const pathname = usePathname();
  
  // Set up the URL by concatenating the language and href
  const url = item?.href === "/" ? `/${lang}` : `/${lang}${item?.href}`;
  
  // Check if the current route (pathname) matches the link URL
  ...
  return (
    <Link
      href={url}  // Ensure the href is set to the constructed URL
      className={cn(
        ...
        <div className="flex flex-col gap-2">
          {selectedMenu.menuItems.map((menu) => (
            <Fragment key={menu.name}>
              {menu.href ? (
                <LinkMenuItem item={menu} lang={lang} />
              ) : (
                <CollapsibleMenuItem item={menu} lang={lang} />
              )}
            </Fragment>
          ))}
        </div>
