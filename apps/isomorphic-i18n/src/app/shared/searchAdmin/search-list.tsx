...
// import  from './page-links.data';
export default function SearchList({ onClose }: { onClose?: () => void } ,{ lang }: { lang?: string }) {
  const inputRef = useRef(null);
  const [searchText, setSearchText] = useState('');
  const { t } = useTranslation(lang!, "nav");
  const links = PageLinks({ lang });  
  let menuItemsFiltered = links;
  if (searchText.length > 0) {
    menuItemsFiltered = links.filter((item: any) => {
            ...
            <Fragment key={item.name + '-' + index}>
              {item?.href ? (
                ...
                <Title
                  as="h6"
                  className={cn(
                    'mb-1 px-3 text-xs font-semibold dark:text-white uppercase tracking-widest text-gray-500 ',
